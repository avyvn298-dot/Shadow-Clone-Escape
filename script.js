/* =========================
   script.js — Shadow Clone Escape (Polished AAA)
   - Place this file in your repo root (replace previous script.js)
   - Put assets in `assets/` with the names below or edit ASSET paths
   - Start the game by clicking "Start Run" in the title overlay (user gesture enables audio)
   ========================= */
...
/* -------------------------
   ...
   ------------------------- */
const ASSET = {
  ninja: "assets/ninja_spritesheet.png",       // ninja frames (e.g. 1536x534, 4 frames)
  clone: "assets/clones_spritesheet.png",      // clones frames (e.g. 1060x433, 3 frames)
  portal: "assets/portal.png",                 // portal image (361x316)
  background: "background.png",                // background image (root)
...
/* -------------------------
   ...
   ------------------------- */
const STORAGE = {
  SETTINGS: 'shadow_clone_settings_v2',
  BEST: 'shadow_clone_best_v2',
  LEADER: 'shadow_clone_leader_v2'
};
...
} catch (e) { console.warn('Could not parse saved settings', e); }
...
const LEVELS = [
  ...
  { name: "Wandering Echo", scale: 1.15, cloneSpeed: 1.12, powerupRate: 0.02 },
  { name: "Night Stalker", scale: 1.3, cloneSpeed: 1.25, powerupRate: 0.016 },
  { name: "Spectral Onslaught", scale: 1.45, cloneSpeed: 1.45, powerupRate: 0.012 },
  { name: "Ninja's Dread", scale: 1.65, cloneSpeed: 1.7, powerupRate: 0.009 },
  { name: "Endless", scale: 1.9, cloneSpeed: 2.0, powerupRate: 0.007 }
];

/* -------------------------
   Asset containers
   ------------------------- */
const IMG = { ninja: null, clone: null, portal: null, background: null };
const SFX = { bg: null, spawn: null, powerup: null, portal: null, death: null };

/* Sprite frame meta (will be set after load) */
const SPRITE = {
...
/* Preload with progress updates (used by Start button) */
async function preloadAll(showProgress = true) {
  const tasks = [
  ...
  const setProgress = (pct, msg) => {
    ...
    if (progressText) progressText.textContent = msg || `Loading ${pct}%`;
  };

  for (const t of tasks) {
    if (!t.path) { completed++; setProgress(Math.round((completed / total) * 100)); continue; }
    if (t.type === 'img') {
      const i = await loadImage(t.path);
      if (i) {
        if (t.key === 'ninja') IMG.ninja = i;
        if (t.key === 'clone') IMG.clone = i;
        if (t.key === 'portal') IMG.portal = i;
        if (t.key === 'background') IMG.background = i;
      ...
      if (a) {
        if (t.key === 'bg') SFX.bg = a;
        if (t.key === 'spawn') SFX.spawn = a;
        if (t.key === 'powerup') SFX.powerup = a;
        if (t.key === 'portal') SFX.portal = a;
        if (t.key === 'death') SFX.death = a;
    ...
    completed++;
    setProgress(Math.round((completed / total) * 100), `Loading assets… ${Math.round((completed / total) * 100)}%`);
    // small delay to make progress visible (UX)
...
/* compute sprite frame integer dimensions to avoid bleeding */
function initSpriteFrames() {
  if (IMG.ninja) {
    SPRITE.ninja.frameW = Math.floor(IMG.ninja.naturalWidth / SPRITE.ninja.cols);
    SPRITE.ninja.frameH = Math.floor(IMG.ninja.naturalHeight / SPRITE.ninja.rows);
  }
  if (IMG.clone) {
    SPRITE.clone.frameW = Math.floor(IMG.clone.naturalWidth / SPRITE.clone.cols);
    SPRITE.clone.frameH = Math.floor(IMG.clone.naturalHeight / SPRITE.clone.rows);
  }
  if (IMG.portal) {
    SPRITE.portal.frameW = Math.floor(IMG.portal.naturalWidth / SPRITE.portal.cols);
    SPRITE.portal.frameH = Math.floor(IMG.portal.naturalHeight / SPRITE.portal.rows);
  }
}

/* -------------------------
   Canvas sizing & tile grid
   ------------------------- */
...
/* cache the maze drawing to an offscreen canvas for performance */
function cacheMaze() {
  if (!maze) return;
...
/* -------------------------
   Spawn portal at farthest open cell
   ------------------------- */
...
function startLevel(levelIndex) {
  currentLevel = clamp(levelIndex, 0, LEVELS.length - 1);
  const L = LEVELS[currentLevel];

  // recompute grid size scaled by level
  resizeCanvas();
  const baseCols = 19, baseRows = 19;
  cols = Math.max(11, Math.floor(baseCols * L.scale));
  rows = Math.max(11, Math.floor(baseRows * L.scale));
  ...
  if (statusText) statusText.textContent = `Level: ${L.name}`;
}
...
function drawBackground(now) {
  ...
  if (IMG.background) {
    ctx.drawImage(IMG.background, 0, 0, IMG.background.naturalWidth, IMG.background.naturalHeight, -40, -20, w + 80, h + 40);
  } else {
    const g = ctx.createLinearGradient(0, 0, 0, h);
    g.addColorStop(0, '#061017'); g.addColorStop(1, '#020406');
    ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
    // subtle moving stars for depth
    ctx.fillStyle = 'rgba(255,255,255,0.02)';
    for (let i = 0; i < 22; i++) {
      const x = (i * 73 + Math.sin(now / 600 + i) * 20) % w;
      const y = (i * 43 + Math.cos(now / 400 + i) * 18) % h;
...
function drawPortal(now) {
  ...
  if (IMG.portal) {
    ...
    ctx.drawImage(IMG.portal, -tileSize * scale / 2, -tileSize * scale / 2, tileSize * scale, tileSize * scale); ctx.restore();
  } else {
    ctx.save(); ctx.translate(px, py); ctx.rotate(rot / 1.6); ctx.fillStyle = '#66ffcc'; ctx.globalAlpha = 0.95;
    ctx.beginPath(); ctx.ellipse(0, 0, tileSize * 0.42, tileSize * 0.5, 0, 0, Math.PI * 2); ctx.fill(); ctx.restore();
  }
}

/* mini map (offscreen canvas approach disabled — we draw simple mini) */
function drawMiniMap() {
  // small minimap drawn into a corner
  const mmW = 140, mmH = 80;
  ctx.save();
  ctx.globalAlpha = 0.88;
  ctx.fillStyle = 'rgba(7,7,7,0.9)';
  ctx.fillRect(12, 12, mmW, mmH);
  const cw = mmW / cols, ch = mmH / rows;
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ctx.fillStyle = maze[y][x] === 1 ? '#222' : '#070707';
      ctx.fillRect(12 + x * cw, 12 + y * ch, cw, ch);
    }
  }
  for (const c of clones) {
    ctx.fillStyle = c.type === 'wraith' ? '#ff66ff' : c.type === 'fast' ? '#ffb86b' : '#ff6666';
    ctx.fillRect(12 + c.x * cw, 12 + c.y * ch, Math.max(1, cw * 0.8), Math.max(1, ch * 0.8));
  }
  ctx.fillStyle = '#66ff99';
  ctx.fillRect(12 + player.x * cw, 12 + player.y * ch, Math.max(1, cw * 0.8), Math.max(1, ch * 0.8));
  ctx.globalAlpha = 1;
  ctx.restore();
}
...
function animate(now) {
  if (!running || paused) return;
  const dt = (now - lastRender) / 1000; lastRender = now; frameCount++;

  // spawn powerups occasionally
  if (frameCount % 900 === 0 && Math.random() < (LEVELS[currentLevel]?.powerupRate || 0.02)) spawnPowerup();
  ...
  for (let i = clones.length - 1; i >= 0; i--) {
    ...
    if (Math.round(c.x) === player.x && Math.round(c.y) === player.y) {
      if (!(activePower && activePower.type === 'cloak' && Date.now() < activePower.until)) {
        ...
        setTimeout(() => { startLevel(Math.max(0, currentLevel)); }, 1000);
        return;
      }
    }
  }
  ...
  if (IMG.ninja) {
    ...
    ctx.save(); const pulse = 0.9 + Math.sin(Date.now() / 420) * 0.08;
    ctx.shadowBlur = 18 * pulse; ctx.shadowColor = 'rgba(50,255,150,0.12)'; ctx.fillStyle = player.color;
...
/* -------------------------
   Level transition (geometry dash-like)
   ------------------------- */
function transitionToNextLevel() {
  ...
  function step() {
    ...
    ctx.restore();
    ctx.fillStyle = `rgba(0,0,0,${t / dur * 0.96})`;
    ctx.fillRect(0, 0, cols * tileSize, rows * tileSize);
    t++;
    if (t <= dur) requestAnimationFrame(step);
    else {
      // go to next level (cap at last)
      startLevel(Math.min(LEVELS.length - 1, currentLevel + 1));
      running = true; lastRender = performance.now(); requestAnimationFrame(animate);
      showNotif(`Level Up: ${LEVELS[currentLevel].name}`);
    }
  }
  step();
}
...
function showNotif(text) {
  const area = document.getElementById('notifArea'); if (!area) return;
...
/* -------------------------
   UI bind & control wiring
   - important: settings modal hidden by default via CSS (.panel-hidden)
   - all open/close use classList to avoid inline style mismatch
   ------------------------- */
function wireUI() {
  // Start button — loads assets then starts
  ...
  continueBtn?.addEventListener('click', () => {
    titleOverlay && (titleOverlay.style.display = 'none');
    if (!player) startLevel(0);
    running = true; lastRender = performance.now(); requestAnimationFrame(animate); tickLoop();
  });

  tutorialBtn?.addEventListener('click', () => { tutorialModal && tutorialModal.classList.remove('panel-hidden'); });
  closeTutorial?.addEventListener('click', () => { tutorialModal && tutorialModal.classList.add('panel-hidden'); });

  settingsBtn?.addEventListener('click', () => { settingsModal && settingsModal.classList.remove('panel-hidden'); });
  closeSettings?.addEventListener('click', () => { // apply & close
    applySettingsFromUI();
    settingsModal && settingsModal.classList.add('panel-hidden');
  });
  ...
  restartBtn?.addEventListener('click', () => {
    ...
    startLevel(0);
    if (SETTINGS.music && SFX.bg) safePlay(SFX.bg, SETTINGS.musicVolume);
    lastRender = performance.now(); running = true; requestAnimationFrame(animate); tickLoop();
  });

  pauseBtn?.addEventListener('click', () => togglePause());
  document.getElementById('menuBtnHeader')?.addEventListener('click', () => { titleOverlay && (titleOverlay.style.display = (titleOverlay.style.display === 'none' ? 'block' : 'none')); });
  ...
  musicToggleEl?.addEventListener('change', (e) => { SETTINGS.music = e.target.checked; saveSettings(); if (!SETTINGS.music && SFX.bg) SFX.bg.pause(); else if (SFX.bg) safePlay(SFX.bg, SETTINGS.musicVolume); });
...
/* -------------------------
   Apply settings from modal controls
   ------------------------- */
function applySettingsFromUI() {
  ...
  saveSettings();
  if (!SETTINGS.music && SFX.bg) SFX.bg.pause();
  else if (SETTINGS.music && SFX.bg) safePlay(SFX.bg, SETTINGS.musicVolume);
}
...
function saveSettings() {
  try { localStorage.setItem(STORAGE.SETTINGS, JSON.stringify(SETTINGS)); } catch (e) { console.warn('Could not save settings', e); }
}

/* -------------------------
   Preview animation (small canvas)
   ------------------------- */
function previewLoop() {
  if (!previewCtx) return;
  previewCtx.clearRect(0, 0, previewCanvas.width, previewCanvas.height);
  previewCtx.fillStyle = '#02030a'; previewCtx.fillRect(0, 0, previewCanvas.width, previewCanvas.height);
  for (let i = 0; i < 8; i++) {
    const x = 20 + i * 44 + Math.sin(Date.now() / 600 + i) * 6;
    previewCtx.fillStyle = `rgba(102,255,153,${0.2 + (i % 2) * 0.25})`;
    previewCtx.beginPath(); previewCtx.arc(x, previewCanvas.height / 2, 10, 0, Math.PI * 2); previewCtx.fill();
  }
...
function togglePause() {
  ...
  else { running = true; pauseBtn && (pauseBtn.textContent = '⏸ Pause'); if (SETTINGS.music && SFX.bg) safePlay(SFX.bg, SETTINGS.musicVolume); lastRender = performance.now(); requestAnimationFrame(animate); tickLoop(); }
}

/* -------------------------
   Boot sequence: wire UI, start preview, and prefetch quietly
   ------------------------- */
async function boot() {
  resizeCanvas();
  wireUI();
  previewLoop();
  updateLeaderboardUI();
  // Prefetch some assets quietly so title preview looks better (no progress UI)
...
/* -------------------------
   Expose debug tools
   ------------------------- */
window.__SHADOWCLONE = { IMG, SFX, startLevel, preloadAll, SETTINGS, addToLeaderboard };

/* Shadow Clone Escape — Polished v3
   Modular, optimized, level + portal + lifespans + popups + audio triggers.
   Drop into repo root with index.html/style.css and enable GitHub Pages.
*/
...
/* LEVEL + PORTAL + MESSAGES */
let LEVEL = 1;
let PORTAL = null;
const CLONE_LIFESPAN_TICKS = 60 * 40; // ~40s
let messages = [];
const LEVEL_COLORS = ['#0b0b0b','#101522','#1a141b','#122018','#17121e'];

/* -------------------- helpers -------------------- */
function randInt(a,b){ return Math.floor(Math.random()*(b-a+1))+a; }
function shuffle(a){ for(let i=a.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [a[i],a[j]]=[a[j],a[i]];} return a; }
function nowSec(){ return Math.floor((Date.now() - startTime)/1000); }
function addFloatingText(text, tileX, tileY, color='white'){
  const px = (tileX + 0.5) * tileSize; const py = (tileY + 0.45) * tileSize;
  messages.push({ text, x: px, y: py, alpha: 1, color });
}
...
function resetGame(){
  ...
  // spawn portal for current level (keeps resetGame untouched; we explicitly call spawnPortal after reset)
}

/* -------------------- PORTAL & LEVEL helpers (ADDED) -------------------- */
...
function drawPortal(){
  if(!PORTAL) return;
  const cx = (PORTAL.x + 0.5) * tileSize;
  const cy = (PORTAL.y + 0.5) * tileSize;
  const pulse = 0.85 + 0.25 * Math.sin(frameCount / 12);
  ctx.save();
  ctx.globalAlpha = 0.12 + 0.06 * Math.sin(frameCount / 10);
  ctx.beginPath();
  ctx.fillStyle = 'rgba(80,200,255,0.14)';
  ctx.arc(cx, cy, tileSize * 0.9 * pulse, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;
  ctx.fillStyle = 'cyan';
  ctx.fillRect(PORTAL.x * tileSize + 2, PORTAL.y * tileSize + 2, tileSize - 4, tileSize - 4);
  ctx.restore();
}
function advanceLevel(){
  LEVEL += 1;
  addFloatingText(`LEVEL ${LEVEL}`, PORTAL.x, PORTAL.y, '#ffd86b');
  cloneInterval = Math.max(30, cloneInterval - 60);
  clones = []; movesHistory = []; powerups = []; particlesArr = [];
  maze = generateMaze(cols, rows);
  spawnPortal();
...
function applyPowerup(type){
  ...
  try{ addFloatingText(`${type.toUpperCase()}!`, player.x, player.y, '#7af'); }catch(e){}
}

/* -------------------- clones (existing behavior plus lifespans) -------------------- */
class Clone {
...
document.addEventListener('keydown', (e)=>{
  if(!running) return;
  if(e.key==='ArrowUp' || e.key==='w'){ activeDirs.up = true; stepPlayer(); try{ if(SETTINGS.sfx) spawnSfx.currentTime=0; }catch(e){} }
  if(e.key==='ArrowDown' || e.key==='s'){ activeDirs.down = true; stepPlayer(); try{ if(SETTINGS.sfx) spawnSfx.currentTime=0; }catch(e){} }
  if(e.key==='ArrowLeft' || e.key==='a'){ activeDirs.left = true; stepPlayer(); try{ if(SETTINGS.sfx) spawnSfx.currentTime=0; }catch(e){} }
  if(e.key==='ArrowRight' || e.key==='d'){ activeDirs.right = true; stepPlayer(); try{ if(SETTINGS.sfx) spawnSfx.currentTime=0; }catch(e){} }
});
...
function spawnClone(){
  ...
  try{ addFloatingText(`${type.toUpperCase()} clone appeared!`, c.x, c.y, '#ffa500'); }catch(e){}
}
...
function gameOverSequence(){
  ...
  if(elapsed > prevBest){
    ...
    if(SETTINGS.sfx) try{ /* play new record */ }catch(e){}
...
function gameOver(){ gameOverSequence(); }
...
function drawMaze(){
  ...
  ctx.fillStyle = LEVEL_COLORS[(LEVEL - 1) % LEVEL_COLORS.length];
...
function animate(now){
  ...
  // cleanup expired clones cheaply
  clones = clones.filter(c => (frameCount - (c.spawnFrame || 0)) < CLONE_LIFESPAN_TICKS);
  ...
  // particles & messages & minimap & HUD
  drawParticles(ctx);
  updateMessages(); drawMessages();
  drawMiniMap();
  updateHUD();
...
/* -------------------- UI bindings -------------------- */
function safePlayAudio(audioEl){ try{ audioEl.currentTime = 0; audioEl.play(); }catch(e){} }

function startRun(){
  LEVEL = 1; levelBox.textContent = `Level: ${LEVEL}`;
  saveSettings();
  ...
  spawnPortal();
  if(SETTINGS.music){ safePlayAudio(bgMusic); }
...
restartBtn.addEventListener('click', ()=>{ resetGame(); spawnPortal(); if(SETTINGS.music){ safePlayAudio(bgMusic); } lastFrame = performance.now(); requestAnimationFrame(animate); });
menuBtn.addEventListener('click', ()=>{ running = false; document.getElementById('menu').style.display = 'block'; document.getElementById('ui').classList.add('panel-hidden'); mobileControls.classList.add('hidden'); try{ bgMusic.pause(); }catch(e){} });
